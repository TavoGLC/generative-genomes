...
trainIds = pd.read_csv(outputPath+'/trainids.csv')
testIds = pd.read_csv(outputPath+'/testids.csv')
...
lr_decayed_fn = tf.keras.optimizers.schedules.CosineDecayRestarts(0.0005,
                                                                  stepsperepoch//2,
                                                                  t_mul=1.5,
                                                                  m_mul=0.45,
                                                                  alpha=0.2)
...
model.layers[2].save(outputPath+'/decoder.h5')
...
axs[1].plot(model.history.history['MeanHammingDistance'],label='MeanHammingDistance')
axs[1].plot(model.history.history['val_MeanHammingDistance'],label='val_MeanHammingDistance')
axs[1].legend(loc=1)

axs[2].plot(model.history.history['MinHammingDistance'],label='MinHammingDistance')
axs[2].plot(model.history.history['val_MinHammingDistance'],label='val_MinHammingDistance')
axs[2].legend(loc=1)

axs[3].plot(model.history.history['MaxHammingDistance'],label='MaxHammingDistance')
axs[3].plot(model.history.history['val_MaxHammingDistance'],label='val_MaxHammingDistance')
axs[3].legend(loc=1)

plt.tight_layout()
plt.savefig(outputPath+'/figTrainingSmall.png')
plt.close()
