...
MetaData = pd.read_csv('/media/tavo/storage/biologicalSequences/genomes/hiv-1/sequences.csv')
...
seqsfile = SeqIO.parse("/media/tavo/storage/biologicalSequences/genomes/hiv-1/sequences.fasta", "fasta")
seqs = []
ids = []
...
def MakeSequenceEncoding(sequence):
    ...
    nToAdd = (24*24*24) - len(stringFrags)
...
def EncoderModel(units=24):
    ...
    InputFunction = Input(shape=(13824,5))
    
    X = Reshape((24,24,24,5))(InputFunction)
...
def DecoderModel(units=24):
    ...
    X = Dense(24*3*units,use_bias=False)(X)
    X = BatchNormalization()(X)
    X = Activation('selu')(X)
    
    X = Reshape((24,3,units))(X)
...
lr_decayed_fn = tf.keras.optimizers.schedules.InverseTimeDecay(0.00025,
                                                               25*stepsperepoch,
                                                               4)
