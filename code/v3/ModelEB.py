...
MetaData = pd.read_csv('/media/tavo/storage/biologicalSequences/genomes/eb/sequences.csv')
...
seqsfile = SeqIO.parse("/media/tavo/storage/biologicalSequences/genomes/eb/sequences.fasta", "fasta")
seqs = []
ids = []
...
def MakeSequenceEncoding(sequence):
    ...
    nToAdd = (48*64*64) - len(stringFrags)
...
def EncoderModel(units=24):
    ...
    InputFunction = Input(shape=(196608,5))
    
    X = Reshape((48,64,64,5))(InputFunction)
...
def DecoderModel(units=24):
    ...
    X = Dense(48*2*units,use_bias=False)(X)
    X = BatchNormalization()(X)
    X = Activation('selu')(X)
    
    X = Reshape((48,2,units))(X)
